...
# Gets the members of a team, the game acronym and the stats data, then it calculates the average rank of that team
def getAvgRank(members, gameAcronym, players):
...
# Makes a list of discord.Member's that are tagged in a message, message it's stored as string and tagged members are received as <@id>
def getMembers(str):  
...
# Gets a discord id and checks if that id is already on the database, if not it will add it. Gotta add to make it add depending on the game acronym
def check(member):  
    playerid = member.id
    ...
    # Checks if the player id is in database, if it is then it returns
...
# Gets the match result (win/tie), and appends the new stats file with the date and detail to the registry.json file
async def regStats(a, gameAcronym, membersA, membersB):
...
# Calculates delta on ELO rank
def deltaelo(memberA, teamBrank, gameAcronym, S, players):  
    gameAcronym = gameAcronym.lower()
    RA = 0.
...
# Sends Current bot's ping
@bot.command()
async def ping(ctx):  
    await ctx.send(f'Pong! {round(bot.latency*1000)}ms')


# Checks tagged user's stats
@bot.command()
async def stats(ctx, gameAcronym, member: discord.Member):  
    # await role(member)
    gameAcronym = gameAcronym.lower()
    check(member)
...
# Takes current message author's and tagged user's stats, checks them and calculates new ones and dumps in file
@bot.command()
async def ranked(ctx, cond, gameAcronym):
    ...
    async with ctx.typing():
        ...
        teamArank = getAvgRank(membersA, gameAcronym,
                               players) * len(membersA)/len(membersB)
        teamBrank = getAvgRank(membersB, gameAcronym,
                               players) * len(membersB)/len(membersA)
